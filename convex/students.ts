...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const submittedStudentEmail = args.email?.trim();
    const parentEmail = args.parentEmail?.trim();

    if (!submittedStudentEmail && !parentEmail) {
      throw new Error("Please provide at least a student email or a parent email");
    }

    if (parentEmail && submittedStudentEmail && parentEmail === submittedStudentEmail) {
      throw new Error("Parent email must be different from student email");
    }

    const studentEmail = submittedStudentEmail ?? `student-${nanoid(12).toLowerCase()}@no-login.local`;
    ...
    if (parentEmail) {
      ...
      const parentIsPrimary = submittedStudentEmail
        ? (args.parentIsPrimary ?? true)
        : true;
      ...
      await ctx.db.insert("parentAccessCodes", {
        code,
      ...
      // @ts-expect-error Internal API types are regenerated by Convex codegen.
      await ctx.scheduler.runAfter(0, internal.emails.sendParentInviteEmail, {
        parentEmail,
        parentName: existingParentUser?.name ?? `Parent of ${args.name}`,
